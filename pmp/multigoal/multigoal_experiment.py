...
class MultigoalExperiment(Experiment):
    ...
    def run(self, visualization=False, n=1, n_start=1, cplex_trials=1, methods=None, criterion='max_appr', k_cc=None,
        ...
        while i <= n * len(methods):
            ...
            if save_in:
                multigoal_save_to_file(self, FileType.IN_FILE, i_per_method, candidates, voters, overwrite=False)
            if save_out:
    ...
    def __run_election(self, candidates, preferences, method='ILP', criterion='max_appr', k_cc=None):
        seed()
        ...
        return self.rule(self.thresholds).find_committee(self.k, profile, method=method, criterion=criterion, k_cc=k_cc)
