from .rule import Rule
from .weakly_separable import WeaklySeparable
from .bloc import Bloc
from .borda import Borda
from .sntv import SNTV
from .chamberlin_courant import ChamberlinCourant

from .multigoal_rule import MultigoalRule
from .multigoal_cc_bloc import MultigoalCCBloc
from .multigoal_cc_borda import MultigoalCCBorda
from .multigoal_cc_sntv import MultigoalCCSNTV
from .multigoal_multibloc import MultigoalTBloc
from .multigoal_bloc_borda import MultigoalBlocBorda
from .multigoal_bloc_sntv import MultigoalBlocSNTV
from .multigoal_borda_sntv import MultigoalBordaSNTV
